...
var OPAQUE = Block.createSpecialType({
    solid: true,
    base: 1,
    lightopacity: 15,
    explosionres: 4,
...
function createBlock(id, variations, type){
    if(!variations[0].inCreative){
        return;
    }

    var textureInfo = variations[0].texture;
    var texture = [];

    if(textureInfo.length == 1){
        for(var i = 0; i < 6; i++){
            texture[i] = {name: textureInfo[0][0], data: textureInfo[0][1] || 0};
...
function addShaped(result, recipe, components, callback){
    ...
    for(var i = 0; i < components.length; i += 3){
        dictionary[components[i]] = {id: components[i + 1], data: components[i + 2]}
    }
...
for(var i in scripts){
    ...
    var regexp = /var\s+([a-zA-Z0-9_]*)\s*=\s*Block\.createSpecialType/g;
    var result = null;
...
function findItemTexture(destination, texture){
    ...
    if(textures.length > 0){
        File.copy(textures[0], destination + texture.file);
        return;
    }
    ...
    File.copy("textures\\ic_default.png", destination + texture.file);
}
...
File.createDirectory("ModDocker\\" + project + "\\items-opaque\\");
File.createDirectory("ModDocker\\" + project + "\\terrain-atlas\\");
File.createDirectory("ModDocker\\" + project + "\\vanilla\\");
...
for(var i in items){
    ...
    html += "</div>";
    findItemTexture("ModDocker\\" + project + "\\items-opaque\\", item.texture);
}
...
for(var i in blocks){
    ...
    for(var i = 0; i < 6; i++){
        ...
        findBlockTexture("ModDocker\\" + project + "\\terrain-atlas\\", block.texture[i]);
    }
    ...
    if(block.type != "unknown"){
        for(var i in block.type){
            html += BLOCK_PARAM[i] + ": " + block.type[i] + "<br/>";
        }
    }
...
function getSlot(item){
    ...
    if(typeof item.id == "number"){
        ...
        File.copy("icons\\items\\" + vanillaFile, "ModDocker\\" + project + "\\vanilla\\" + vanillaFile);
...
File.copy("textures\\craft-bg.png", "ModDocker\\" + project + "\\craft-bg.png");
File.copy("textures\\craft-arrow.png", "ModDocker\\" + project + "\\craft-arrow.png");
...
File.createDirectory("ModDocker");

// Write HTML
File.write("ModDocker\\" + project + ".html", html);
